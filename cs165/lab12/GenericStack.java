...
public class GenericStack<T>{
    ...
    public GenericStack(int capacity) {
        genericStack = (T[]) new Object[capacity];
        size = 0;
    }
    ...
    public T pop() {
        if (size == 0) {
            ...
            T temp = genericStack[size - 1];
            genericStack[size - 1] = null;
            size--;
            return temp;
        }
    }
     ...
     */
    public T push(String s) {
        if (size == genericStack.length) {
            throw new IllegalStateException();
        }
        else {
            genericStack[size] = (T) s;
            size++;
            return (T) s;
        }
    }
